...
public class SwipeCard: UIView, UIGestureRecognizerDelegate {
	...
	func handlePan(_ recognizer: UIPanGestureRecognizer) {
		...
		if recognizer.state == .changed {
			...
			prev.setCompleteState(swipecomplete)
			nxt.setCompleteState(swipecomplete)
			if swipecomplete {
			...
			if swipecomplete {
				...
				UIView.animate(withDuration: 0.35, delay: 0.0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0.6, options: UIViewAnimationOptions.curveEaseOut, animations: {
					self.frame = orginalFrame
						...
						//TODO: is there a need to execute something else here?
				})
			}
